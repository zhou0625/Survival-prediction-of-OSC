...
svc_64 = SVC(kernel='linear', class_weight="balanced", random_state=300, probability=True)
svc_64_clinic = SVC(kernel='linear', class_weight="balanced", random_state=300, probability=True)
svc_ROI = SVC(kernel='linear', class_weight="balanced", random_state=300, probability=True)
svc_ROI_clinic = SVC(kernel='linear', class_weight="balanced", random_state=300, probability=True)

svc_64.fit(X_train_64_scaled, y_train_64)
svc_64_clinic.fit(X_train_64_clinic_scaled, y_train_64_clinic)
svc_ROI.fit(X_train_ROI_scaled, y_train_ROI)
svc_ROI_clinic.fit(X_train_ROI_clinic_scaled, y_train_ROI_clinic)


y_test_pred_64 = svc_64.predict(X_test_64_scaled)
y_test_pred_64_clinic = svc_64_clinic.predict(X_test_64_clinic_scaled)
y_test_pred_ROI = svc_ROI.predict(X_test_ROI_scaled)
y_test_pred_ROI_clinic = svc_ROI_clinic.predict(X_test_ROI_clinic_scaled)
...
# 获取四种情况下的预测概率
probas_64 = svc_64.predict_proba(X_test_64_scaled)[:, 1]
probas_64_clinic = svc_64_clinic.predict_proba(X_test_64_clinic_scaled)[:, 1]
probas_ROI = svc_ROI.predict_proba(X_test_ROI_scaled)[:, 1]
probas_ROI_clinic = svc_ROI_clinic.predict_proba(X_test_ROI_clinic_scaled)[:, 1]
